# Informe técnico de errores y mejoras — Ecosistema `heatmap_stock`
...
> **Este informe sustituye y corrige** el borrador `Roadmap_Integral_de_Mejoras___Ecosistema.md` (incompleto) e incorpora el diagnóstico de campos del 2026-09-19 y los dos `config.py`. Las rectificaciones respecto a esos documentos están en el [Anexo F](#anexo-f--errata-y-contrastes).
...
4. [Modelo de tiempo y ventana operativa](#4-modelo-de-tiempo-y-ventana-operativa)
5. [Hechos técnicos verificados](#5-hechos-técnicos-verificados)
6. [Catálogo de errores y defectos](#6-catálogo-de-errores-y-defectos)
7. [Catálogo de mejoras](#7-catálogo-de-mejoras)
8. [Incongruencias entre fuentes](#8-incongruencias-entre-fuentes)
9. [Hipótesis y estado de la evidencia](#9-hipótesis-y-estado-de-la-evidencia)
10. [Dudas abiertas y decisiones pendientes](#10-dudas-abiertas-y-decisiones-pendientes)
11. [Plan por fases e hitos](#11-plan-por-fases-e-hitos)
12. [Trazabilidad hipótesis → decisión → error → mejora](#12-trazabilidad)
13. [Riesgos](#13-riesgos)
14. [Criterios de aceptación y KPIs](#14-criterios-de-aceptación-y-kpis)
15. [Anexos](#15-anexos) (incluye [Anexo G · Evaluación de la segunda revisión externa](#anexo-g--evaluación-de-la-segunda-revisión-externa-2026-09-21), v1.1)
...
| Datos y pruebas | `bd_heatmap_informe_2026-09-18.md` (estado de la BD); `diagnostico_scrapper_v5_al_2026-09-19.md` y una segunda evaluación del 2026-09-22 (narrativas de dos corridas del script siguiente, sábado 05:30 UTC y martes 04:23 UTC); `wztest_scraper_timeframes.py` (código fuente del script, incorporado el 2026-09-22 — permite verificar la metodología de ambas corridas, no sus valores puntuales, ver §5.2.1) |
...
**Nota de trazabilidad (aplica a esta tabla y a la §5.2.1):** el script (`wztest_scraper_timeframes.py`) sí forma parte del material analizado (§2.1) y su lectura confirma que la metodología descrita es coherente con lo que el código puede producir. Lo que **no** forma parte del material analizado es el *log crudo* de estas dos corridas: los valores concretos de esta tabla (RSI 64,25, volumen 2.318, etc.) provienen de un resumen narrado de quien ejecutó el script, no de una salida que yo haya podido inspeccionar directamente. Se mantienen como 🟡/✅ según la solidez del razonamiento, no como verificación independiente del dato puntual.

### 5.2.1 Segunda corrida (2026-09-22 04:23 UTC) y verificación de metodología

Con el script ya disponible se pudo confirmar, **desde el código y no solo desde el reporte de la corrida**:

| Punto verificado en el código | Confirma o matiza |
|---|---|
| `BASE_FIELDS` del script coincide exactamente en composición con `CAMPOS` de `scraper_live_tradingview_v5.py` (orden distinto, mismo conjunto de 9 campos) | Los hallazgos del script son transferibles al radar de producción en cuanto a qué campos existen |
| `test_timeframes()` (Test B) **no llama a `time.sleep()` entre timeframes**: las 6 peticiones por símbolo (`None, 5, 15, 30, 60, 1D`) se hacen una tras otra sin pausa | Confirma que la comparación entre TF de una misma corrida es casi simultánea (segundos, no minutos), lo que sostiene la lectura mecánica de §2.3 del documento recibido (`volume\|30 = volume\|60` por compartir inicio de barra) |
| `missing = [f for f in BASE_FIELDS if f"{f}{suffix}" not in data]` distingue **campo ausente** de **campo presente con valor `null`** (`.get()` no distingue ambos casos al construir `results`) | No puedo confirmar, sin el log crudo, si `Perf.W\|TF` y los campos `\|1D` aparecieron como "ausentes" (el script habría impreso una advertencia explícita) o como "presentes pero nulos". La distinción importa: lo primero indica que el endpoint ignora el sufijo; lo segundo, que lo acepta pero no tiene valor que devolver |
| `DEFAULT_SYMBOLS = ["BINANCE:BTCUSDT", "NASDAQ:AAPL", "FX:EURUSD"]` | El símbolo de FX usado en ambas corridas (`FX:EURUSD`) **no coincide** con ninguno de los configurados en `CONFIG_ACTIVOS` (`FX_IDC:EURUSD` primario, `OANDA:EURUSD` respaldo). Son tres identificadores de TradingView potencialmente con distinto proveedor de datos y semántica de `volume`. Los hallazgos de EURUSD (incluido el volumen que cambia con el precio congelado, §5.6) deben tratarse como una señal a confirmar con el símbolo real, no como un hecho de producción (Q23) |
| La segunda corrida (2026-09-22 04:23 UTC = 00:23 ET) ocurre con NYSE **totalmente cerrada** (ni siquiera pre-market) | Cualquier patrón de "vela en formación" observado en AAPL en ese horario puede reflejar el arrastre de la sesión extendida anterior en vez de actividad real; no generaliza automáticamente a la sesión regular, que es la que importa para 15 min (ver H19) |
| Cálculo de calendario independiente (no depende del reporte recibido): 2026-09-19 es **sábado** (semana ISO 38) y 2026-09-22 es **martes** (semana ISO 39) — hay un lunes de por medio | Si `Pivot.M.Camarilla.R3` base fuera **semanal** con reinicio habitual en lunes, el valor debería haber cambiado entre ambas corridas. Que se reporte idéntico en ambas fechas es, por tanto, evidencia algo más fuerte contra la hipótesis semanal de lo que el resumen recibido reconoce (ver H22) |

**Resultado:** la metodología del script es sólida y explica por qué los patrones observados (agrupación por pares en `Pivot.M.Camarilla.R3\|TF`, `volume\|30 = volume\|60`) son mecánicamente esperables y no artefactos de medición. Los valores concretos de la segunda corrida siguen sin log crudo verificable por mí; ver H19, H22, H24 (§9) para el estado actualizado de cada hipótesis y Q22–Q24 (§10.1) para lo que queda pendiente.
...
| `volume` (equity/ETF) | **Acumulado del día**; se reinicia con la sesión | Volumen de la vela actual o última (H19) | Base: diferenciar entre ticks. `\|TF`: solo con regla de cierre de barra |
| `volume` (FX, p. ej. EURUSD) | Acumulado que **sigue creciendo con el precio congelado** fuera de sesión NYSE (2026-09-22: 21.933 → 21.949 con `close` fijo, según reporte de la segunda corrida) | Sin verificar | Semántica distinta de equity: probablemente conteo de ticks/cotizaciones más que volumen nocional negociado (Q24); no comparable directamente con `average_volume_10d_calc`/`30d_calc` de una acción |
...
|---|---|---|---|---|---|---|
| **M-CAP-01** | Gate de sesión **por clase de activo** dentro del código (B.1): equity/ETF con calendario NYSE (`dim_trading_session`); FX y cripto con ventana propia (24/5 o 24/7, con corte de mantenimiento), no apagados por el gate de NYSE. El cron queda cada 3 min sin lógica horaria propia. | E-OPS-01, E-HM-03, Q22 | P0 | M | M-DAT-01 | Pruebas de feriado, cierre anticipado y DST pasan para equity/ETF; FX y cripto siguen capturando fuera de la ventana NYSE (por ejemplo, madrugada ET con mercado asiático activo); 0 ticks de equity/ETF fuera de 09:00–16:00 ET |
...
| **M-DSH-05** | RVOL por hora del día tolerante a nulos (solo equity/ETF; para FX, resolver primero Q24 antes de aplicar la misma fórmula). | E-DSH-02 | P2 | M | M-DAT-02 | Mediana ≈ 1,0 a cualquier hora |
...
| **H19** | `volume\|TF` y `change\|TF` describen la **vela en formación**, acumulada desde el inicio de su propio intervalo (no la "última vela cerrada"). | Corrida 1 (BTC, 2026-09-19 05:30 UTC): `volume\|5 = \|15 = \|30`. Corrida 2 (2026-09-22 04:23 UTC, script `wztest_scraper_timeframes.py`, Test B): en BTC, AAPL y EURUSD, `volume\|30 = volume\|60` mientras `\|5` y `\|15` difieren, coherente con que las barras de 30 y 60 min comparten el mismo inicio de hora (:00) y las de 5/15 min no. El script no duerme entre peticiones de distinto TF (confirmado en el código), así que las seis lecturas por símbolo son casi simultáneas y la comparación entre TF es válida. | 🟡 (mecanismo coherente y reproducido dos veces; sigue sin log crudo verificable por mí, y el caso de AAPL corresponde a un momento con el mercado totalmente cerrado — 00:23 ET —, no a una sesión con operativa real, así que el patrón podría reflejar el arrastre de la sesión extendida previa en vez de una vela realmente "en formación"; repetir en sesión regular) | T4 (repetir en sesión regular de AAPL) | Solo se pueden usar con la regla de cierre de barra (M-CAP-03); no extrapolar el mecanismo a acciones sin repetir la prueba con el mercado abierto. |
...
| **H22** | `Pivot.M.Camarilla.R3` base es un nivel de **rango amplio** (probablemente mensual); con sufijo, `\|5/\|15` serían pivotes **diarios** y `\|30/\|60` **semanales**. | Con `R3 = C + 1,1·(H−L)/4`, NVDA (R3 − cierre = 10,56) implica H−L ≈ 38 (~17,5 % del precio) y BTC ≈ 12,6 %: improbables para un día (tomando C ≈ precio actual). Con sufijo, los valores se agrupan en pares idénticos (`\|5 = \|15`, `\|30 = \|60`) en BTC, AAPL y EURUSD, compatible con una selección automática de período según el timeframe. **Refuerzo independiente (verificado por calendario, no solo por el reporte de la corrida):** entre la corrida del 2026-09-19 (sábado, semana ISO 38) y la del 2026-09-22 (martes, semana ISO 39) hay un lunes de por medio — si el pivote base fuera **semanal** con reinicio habitual en lunes, debería haber cambiado entre ambas corridas. El reporte de la segunda corrida indica que el valor base de BTC fue idéntico en ambas fechas pese a que el precio subió ~5,5 % (81,0k → 85,5k), lo que descarta con más fuerza la hipótesis semanal (no solo la diaria) para el campo base. | 🟡, reforzada (el cruce de semana ISO es un argumento independiente de mi parte; el valor concreto reportado —83862,35424999999 idéntico en ambas fechas— sigue sin log crudo verificable por mí) | T6 | Si se confirma, los pivotes diarios de TradingView estarían disponibles en `\|15` sin calcularlos; el base sirve de contexto mensual. |
...
| **H24** | El endpoint refresca **acciones en horario** cada 10–30 s. | Solo medido en BTC (tiempo real); AAPL y EURUSD estaban cerrados en ambas corridas de prueba. Corrida 2 (2026-09-22 04:23 UTC, madrugada de baja liquidez): en BTC, 2 valores únicos en 41 s (≈ un cambio cada ~40 s), frente a 3 valores únicos en 50 s de la corrida 1 (2026-09-19 05:30 UTC). El intervalo entre valores distintos varía con el momento del día incluso dentro de un mismo símbolo 24/7. | 🔴 (nada de esto mide acciones en horario, que es lo que importa para 15 min; matiz aceptado: el intervalo no es constante) | T7 | El "intervalo entre valores distintos" observado en BTC/EURUSD podría reflejar el ritmo de nuevas operaciones en el mercado subyacente y no necesariamente la cadencia de refresco del propio endpoint (ambas explicaciones son indistinguibles desde fuera con una sola serie de peticiones); no cambia la necesidad de T7, pero sí la interpretación causal. |
...
| **Q22** | ¿El gate de sesión (M-CAP-01) debe aplicarse distinto por clase de activo — equity/ETF con ventana NYSE, FX/cripto 24/5 o 24/7 — en vez de un gate único para los 110 símbolos? | La segunda corrida del script de diagnóstico (2026-09-22 04:23 UTC) reporta que el `volume` de EURUSD cambió (21.933 → 21.949) mientras `close` permaneció fijo, en un horario en que NYSE está cerrada pero Asia/Europa operan. Un gate global apagaría la captura de FX y cripto fuera de la ventana NYSE, perdiendo precisamente la actividad que mueve esos activos. | Decidir alcance de M-CAP-01: aplicar por `asset_class` (equity/ETF → `dim_trading_session`; FX/cripto → ventana propia, por ejemplo 24/5 con corte de mantenimiento) |
| **Q23** | ¿Los hallazgos de EURUSD del script de diagnóstico son válidos para el símbolo real de producción? | `wztest_scraper_timeframes.py` usa `FX:EURUSD` como símbolo por defecto; `CONFIG_ACTIVOS` usa `FX_IDC:EURUSD` (primario) y `OANDA:EURUSD` (respaldo) — tres identificadores distintos en TradingView, potencialmente con distinto proveedor de datos, `update_mode` y semántica de `volume`. | Repetir el Test A/B del script pasando `--symbol FX_IDC:EURUSD --symbol OANDA:EURUSD` antes de dar por buenos los hallazgos de EURUSD para producción |
| **Q24** | ¿Qué representa `volume` en un par de FX (`FX:EURUSD`, `FX_IDC:EURUSD`, `OANDA:EURUSD`) — volumen nocional, conteo de ticks/cotizaciones, o un proxy sintético del bróker? | A diferencia de una acción, un par FX descentralizado no tiene un "volumen negociado" único y verificable; si es un proxy de ticks, no es comparable con `average_volume_10d_calc`/`average_volume_30d_calc` de una acción, lo que afectaría a M-DSH-05 (RVOL) si alguna vez se calcula para FX | Documentación de TradingView sobre el campo `volume` en FX, o comparar contra una fuente de FX conocida |
...
`en_ventana()` sigue siendo el gate correcto **para equity y ETF**. Para radar y calendario, cuyo universo mezcla equity/ETF con FX, cripto, futuros y yields (Q22), el gate debe evaluarse por símbolo según su `asset_class`, no una sola vez por ciclo:
...
def en_ventana_fx(ahora: pd.Timestamp | None = None) -> bool:
    """FX opera ~24/5: cerrado solo en el corte de mantenimiento del fin de semana
    (aprox. viernes 22:00 UTC a domingo 22:00 UTC; ajustar al proveedor real)."""
    ahora = ahora or pd.Timestamp.now(tz="UTC")
    dow, hour = ahora.dayofweek, ahora.hour            # 0=lunes … 6=domingo
    if dow == 5:                                       # sábado: cerrado todo el día
        return False
...
GATE_POR_CLASE = {
    "equity": en_ventana_nyse, "etf": en_ventana_nyse,
    "fx": en_ventana_fx, "crypto": en_ventana_cripto,
    "future": en_ventana_nyse,                          # ajustar si se opera el rango nocturno (Q2)
}

def en_ventana(asset_class: str, **kwargs) -> bool:
    return GATE_POR_CLASE.get(asset_class, en_ventana_nyse)(**kwargs)

# En el bucle del radar: filtrar el símbolo, no abortar todo el ciclo.
# if not en_ventana(asset_class_de(symbol)): continue
# El heatmap (solo equity/ETF) sigue usando en_ventana_nyse() una vez por corrida.
# El calendario usa en_ventana_nyse(pre_min=90) para arrancar a las 08:00 ET.
```

Los horarios exactos del corte de mantenimiento de FX (`en_ventana_fx`) dependen del proveedor (OANDA, FX_IDC) y no están verificados contra un calendario oficial; son una aproximación a validar antes de producción.
...
def test_en_ventana_nyse(ts, esperado):
    assert en_ventana_nyse(ahora=pd.Timestamp(ts)) is esperado
```
...
| 1.2 | 2026-09-22 | Incorpora `wztest_scraper_timeframes.py` (ahora sí presente en el material analizado) y una segunda corrida del diagnóstico (2026-09-22 04:23 UTC). Nueva §5.2.1: verifica la metodología del script (sin `sleep` entre timeframes en el Test B, `BASE_FIELDS` idéntico a `CAMPOS`) y detecta que el símbolo de FX usado en las pruebas (`FX:EURUSD`) no coincide con el de producción (`FX_IDC:EURUSD`/`OANDA:EURUSD`). Refuerza H22 con un argumento de calendario independiente (cruce de semana ISO). Actualiza H19, H22 y H24 con matices (vela en formación en mercado cerrado, variabilidad del refresco). Añade Q22 (gate por clase de activo), Q23 (símbolo de prueba vs. producción) y Q24 (semántica de `volume` en FX). Actualiza M-CAP-01 y el fragmento B.1 para diferenciar el gate por clase de activo; anota M-DSH-05 |
