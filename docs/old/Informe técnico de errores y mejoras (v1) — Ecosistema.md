...
**Este informe sustituye y corrige** el borrador `Roadmap_Integral_de_Mejoras___Ecosistema.md` (incompleto) e incorpora el diagnóstico de campos del 2026-09-19 y los dos `config.py`. Las rectificaciones respecto a esos documentos están en el [Anexo F](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21).
> 
...
1. [Convenciones](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
2. [Alcance, método y límites](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
3. [Resumen ejecutivo](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
4. [Modelo de tiempo y ventana operativa](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
5. [Hechos técnicos verificados](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
6. [Catálogo de errores y defectos](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
7. [Catálogo de mejoras](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
8. [Incongruencias entre fuentes](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
9. [Hipótesis y estado de la evidencia](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
10. [Dudas abiertas y decisiones pendientes](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
11. [Plan por fases e hitos](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
12. [Trazabilidad hipótesis → decisión → error → mejora](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
13. [Riesgos](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
14. [Criterios de aceptación y KPIs](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
15. [Anexos](https://app.notion.com/p/Informe-t-cnico-de-errores-y-mejoras-Ecosistema-heatmap_stock-3e2ccf9f614d80728d37cc8fae86fa80?pvs=21)
...
Además: **54 mejoras** (§7), **16 incongruencias** (§8) y **24 hipótesis** (§9).
...
- **Hallazgo.** (a) `flush_radar_batch` captura toda excepción y devuelve 0; el llamador hace `batch_buffer.clear()`, así que el ciclo se pierde en BD sin reintento. (b) `DB_WRITE_ENABLED` vale `false` por defecto y `.env` sobrescribe variables de entorno reales: sin `.env` el radar corre en modo CSV sin error. (c) `audit_sync_run` tiene solo 2 filas de `radar_v4`. (d) No hay job de reconciliación CSV → BD.
- **Evidencia.** Código de ambos archivos; informe §5.7.
- **Impacto.** Huecos invisibles en BD (el CSV los conserva); indicadores y barras con huecos; el dashboard mostraría "sin datos" sin causa identificable.
...
#### E-HM-04 · Upserts por símbolo y reescritura de `updated_at`

**S2 · 🟡 · Esfuerzo S** — Objeto: `get_or_create_asset()`.

- **Hallazgo.** Se llama dentro del bucle (1.000 consultas por snapshot) con `ON CONFLICT DO UPDATE SET updated_at = CURRENT_TIMESTAMP`.
- **Evidencia.** Código. Si `execute_query` confirma en cada llamada (no revisado), son además 1.000 commits.
- **Impacto.** ~28.000 reescrituras/día en `dim_asset` (1.083 filas, varios índices): bloat y WAL. `updated_at` pierde significado, y los `COALESCE(existente, nuevo)` nunca actualizan atributos que cambian.
- **Corrección.** M-CAP-12 (cache `symbol → asset_id`, alta masiva con `DO NOTHING`, un commit).
- **Aceptación.** ≤ 3 sentencias SQL de dimensión por snapshot; `updated_at` solo cambia si cambia un atributo.
...
| **E-HM-07** | S3 | `records_failed = fetched - upserted if fetched and upserted else 0`: con `upserted == 0` y `fetched > 0` informa 0 fallos. | B.7 |
...
```sql
SELECT source_discovered_by, count(*) AS n, count(*) FILTER (WHERE logo_id IS NULL) AS sin_logoFROM dim_asset GROUP BY 1;
```
