# Roadmap Integral de Ejecución v2 — Ecosistema heatmap_stock
**Versión:** 2.0 (consolidado tras tests D/E/F/G/H/C del 2026-09-22)

**Objetivo:** herramienta de apoyo a decisiones de trading en 15 min, operativa 09:00–16:00 ET.

**Estado:** 8 hipótesis cerradas, 6 dudas cerradas, 2 errores nuevos, 1 decisión confirmada, 1 decisión nueva pendiente.
...
**Cerradas ✅ (evidencia directa):** H1, H6, H7b, H19, H20, H21, H22, H24, Q15, Q16, Q17, Q22, Q23, Q24.
...
| **ID** | **Sev** | **Descripción** | **Fase** |
| --- | --- | --- | --- |
| **E-RAD-14** | S1 | `FX_IDC:EURUSD` devuelve `volume = 0` | F1.7 |
| **E-RAD-15** | S2 | Ventana ciega de ±15 s en cada frontera de barra | F4.1b |
...
## **FASE 0 · Verificación empírica (cerrada parcialmente)**

**Estado:** F0.2 (protocolo T1–T11) **completado al 90%** el 2026-09-22.

**Pendiente:** T3.1 (semántica de `gap`), A1–A16 (SQL).
...
### **F1.7 · Corregir primario de EURUSD (NUEVO)**
...
CAMPOS = ",".join([BASE_1D, _bloque("5"), _bloque("15"), "update_mode"])
```

Nota: `close|TF` se omite (H20 confirmada: `close|5` y `close|15` son idénticos entre sí y distintos de `close` base; se capturan igual vía `premarket_close`).

**Hecho cuando:** una llamada a `/symbol` con `CAMPOS` devuelve todos los campos.
...
### **F3.6 · Calendario: polling por evento**

**Cierra:** E-CAL-03.

**Duración:** 1 día

**Cambio:** cron cada minuto entre `T−1` y `T+10 min` de eventos de alto impacto, ventana 08:00–16:00 ET.
...
**Cambio:** comparar `RSI|15`, `ADX|15`, `CCI20|15` de TV vs cálculo propio en 5–10 sesiones.
...
| **28 sep – 4 oct** | F2 | Gate por clase; particiones UTC; roles |
| **5–11 oct** | F3.1–F3.5 | Batch (Plan A o B); CAMPOS extendido; trazabilidad |
| **12–18 oct** | F3.6–F3.11 | Calendario; heatmap; auditoría; monitoreo |
| **19–25 oct** | F4.1–F4.5 + F4.1b/c | Barras con `close_quality`; indicadores TF; snapshots |
...
| **Métrica** | **Inicio (2026-09-22)** | **Fin F0+F1 (2026-09-27)** | **Fin F3 (2026-10-18)** | **Fin F7 (2026-12-06)** |
| --- | --- | --- | --- | --- |
| Hipótesis cerradas | 8/24 | 16/24 | 18/24 | 21/24 |
| Dudas cerradas | 6/24 | 12/24 | 18/24 | 22/24 |
| Errores S1 activos | 3 | **1** | 0 | 0 |
| Errores S2 activos | 21 | 18 | 4 | 0 |
